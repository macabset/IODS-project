...
Here is our Boston dataset, which is one of R's own datasets. It consists of Housing values in the suburbs of Boston, USA. It has 14 variables to explain the values of the 506 observations, which consern about the town.
...
summary(Boston)
```
The summary of our 14 variables shows that all but two of our variables are numeric and continuous. We have binary variables chas, which gives 1 if tract is situated by the Charles river, and we also have rad, which gives and index for accesibility for radial highways.
...
We use scale to standardize ou data. When variables have a common scal, we can calculate distances between means. We also want our new scales as data frame instead of matrix.
...
```
Because we have scaled our data, all observations are between integral 10. Now we can work them as one and see the real distances of these groups. This will help us in the clustering part.

##2. Classification

We are going to divide the data with classification method. This is method where we need to know before hand the groups for discriminating groups, and we will use the crime variable for this. 

First we need to adjust our variable and create new crime rates to our crim variable. From scaled Boston dataset we take the continuous variable crim and make it categorical.
...
After our scaling we are able to make new variable crime to our dataset. The crime variable has categories low, med low, med high and high based on quantiles of the crim. So there's 127 observations in the low category, which are all the observations under 25% of all observations in crime. Since the old crim isn't necessary anymore, we can drop that out from our boston scaled.

###2.1 Linear Discriminant Analysis

No we're just about to use our data to divide our data to classes with Linear Discriminant Analysis(LDA).

Before we actually can make a model for classification, we have to make sure that we can test how well our model works: That's why we're making train set to create our model and test set to predict our model to the test set. No I'll divide our dataset, so 80% will be used as train set and 20% to test set.
...
In our lda we have crime as target variable against all other variables. Here we obviously use our train set to create the model. Now we're ready to run the LDA, and it should create a model to predict crime rates using all other variables.
...
Now we scale again our data to use it for clustering method. No we are not using crime for the base for classes but guess it with k-means. 
...
With clustering we do not know the number of proper classes beforehand, so we need to find them out by using distance method. Now I am using the standard euclidean distance method.
