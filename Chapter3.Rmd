...
```
We have 30% "penalty", which means that 30% of our predictions are not correctly classified compared to actual data. 

*Let's quess!*
What if we'd guess that when our model give the prediction, it never guesses right

```{R}
loss_func <- function(class, prob) {
  n_wrong <- abs(class - prob) > 1
  mean(n_wrong)
}
